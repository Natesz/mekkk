...
export default defineEventHandler(async (event) => {
  ...
  try {
    ...
    if (!response.ok) {
      throw createError({ statusCode: response.status, message: data.error?.message || 'DALL-E API error' })
    }

    if (!data.data?.[0]?.url) {
      throw createError({ statusCode: 500, message: 'No image URL received from DALL-E' })
    }
